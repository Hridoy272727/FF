...
def sukses(no1,pro,nam):
  print("    %s[%s%s%s] [%s Sent %s] %sSuccess, mulai %s from %s%s %ssended" % (pu, ku, no1, pu, hi, pu, pro, ku, nam, hi))
...
def input():
  ...
  if len(nom) < 5:
    ...
    print "%s[%s!%s] %sEnter The Number Correctly 0\n%s[%s!%s] %sContoh : 85877162199"%(pu,me,pu,ku,pu,me,pu,ku)
    input()
  else:
    global jum
    jum = int(raw_input("%s[%s?%s] %sEnter Number 1 : "%(pu,me,pu,pu)))
    main()
#-------------------------------SPAM Function-------------------------------------------
def asakita():
...
def klikwa():
 ...
 if json.loads(tes.text)["message"] == 'OTP Sent':
  print "     %s[%s%s%s] [%s Sent %s] %sSuccess, FRee FIre %s from %s%s %ssended %s>> %sSUBSCRIBE%sACINAKONGAME"%(pu,ku,"10",pu,hi,pu,pu,"wa",ku,"klikwa",hi,qu,pu,ku)
 else:
  print "     %s[%s%s%s] [%s Failed %s] %sFailed, FRee FIre %s from %s%s %snot sended %s>> %sSUBSCRIBE DULU %sACINAKONGAME"%(pu,ku,"10",pu,me,pu,pu,"wa",ku,"klikwa",me,qu,pu,ku)
def bakmikeraton():
